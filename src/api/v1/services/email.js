...
const oAth2Client = new google.auth.OAuth2(
  projectConfig.google.clientId,
  projectConfig.google.clientSecret,
  projectConfig.google.redirectUrl
)

oAth2Client.setCredentials({
  refresh_token: projectConfig.google.refreshToken,
})
...
const sendMail = async (to, subject, text) => {
  try {
    ...
    const transport = nodemailer.createTransport({
      ...
      auth: {
        type: "OAuth2",
        user: projectConfig.nodemailer.user,
        pass: projectConfig.nodemailer.pass,
        clientId: projectConfig.google.clientId,
        clientSecret: projectConfig.google.clientSecret,
        refreshToken: projectConfig.google.refreshToken,
        accessToken,
      },
    })

    const mailOptions = {
      from: projectConfig.nodemailer.user,
      to,
      subject,
      html: `<h1>${text}</h1>`,
    }
